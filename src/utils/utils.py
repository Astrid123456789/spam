...
def validate_data_files():
    """Check if required spam data files exist."""
    from .config import DATA_PATH, SMS_TRAIN_FILE, EMAIL_TRAIN_FILE

    logger = get_logger()
    sms_path = DATA_PATH / SMS_TRAIN_FILE
    email_path = DATA_PATH / EMAIL_TRAIN_FILE

    if not sms_path.exists():
        raise FileNotFoundError(f"SMS training file not found: {sms_path}")

    if not email_path.exists():
        raise FileNotFoundError(f"Email training file not found: {email_path}")

    logger.success("Spam data files found")
    return True
